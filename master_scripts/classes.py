...
class DSNT(tf.keras.layers.Layer):
    ...
    def call(self, inputs, method="softmax"):
        ...
        def js_reg_loss(heatmaps, centres, fwhm=1):
            '''
            Calculates and returns the average Jensen-Shannon divergence between
            heatmaps and target Gaussians.
            Arguments:
        ...
        def _softmax2d(target, axes):
            ...
            as this isn't provided by Tensorflow
            Arguments:
        ...
        def make_gaussian(size, centre, fwhm=1):
            ...
            Arguments:
                ...
                fwhm    - Full-width-half-maximum, which can be thought of as a
                            radius.
            '''
            # Scale normalised coordinates to be relative to the size of the frame
